...
export function BudgetChart({ summary }: BudgetChartProps) {
  ...
  const categoriesWithSpending = summary.filter((cat) => parseFloat(cat.total_actual || "0") > 0);
    ...
    return (
      <div className="bg-truehour-card border border-truehour-border rounded-lg p-6">
        ...
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-slate-400 text-center mb-2">No expenses linked yet</p>
  ...
  const dataValues = categoriesWithSpending.map((cat) => parseFloat(cat.total_actual || "0"));
  const colors = categoriesWithSpending.map((cat) => CATEGORY_COLORS[cat.category] || CATEGORY_COLORS["Other"]);
      ...
      <div className="mt-6 pt-6 border-t border-truehour-border">
        <div className="grid grid-cols-2 gap-3 text-sm">
          {categoriesWithSpending.map((cat) => {
            ...
            const total = categoriesWithSpending.reduce((sum, c) => sum + parseFloat(c.total_actual || "0"), 0);
            ...
            return (
              <div key={cat.category} className="flex items-center justify-between">
                ...
                <span className="text-white font-medium text-xs">{percentage.toFixed(0)}%</span>
              </div>
            );
          })}
        </div>
