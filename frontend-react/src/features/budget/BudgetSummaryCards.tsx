...
export function BudgetSummaryCards({ summary }: BudgetSummaryCardsProps) {
  ...
  const totalBudgeted = summary.reduce((sum, cat) => sum + parseFloat(cat.total_budgeted || "0"), 0);

  const totalActual = summary.reduce((sum, cat) => sum + parseFloat(cat.total_actual || "0"), 0);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="bg-truehour-card border border-truehour-border rounded-lg p-6 hover:border-truehour-blue transition-colors">
        ...
        <div className="text-3xl font-bold text-truehour-blue mb-1">{formatCurrency(totalBudgeted)}</div>
        <p className="text-slate-500 text-xs">Allocated for training</p>
      </div>
      ...
      <div className="bg-truehour-card border border-truehour-border rounded-lg p-6 hover:border-truehour-green transition-colors">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className={`${totalActual > totalBudgeted ? "bg-red-500/10" : "bg-green-500/10"} p-2 rounded-lg`}>
            <svg
              className={`w-5 h-5 ${totalActual > totalBudgeted ? "text-red-400" : "text-truehour-green"}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div
          className={`text-3xl font-bold mb-1 ${totalActual > totalBudgeted ? "text-red-400" : "text-truehour-green"}`}
        >
          {formatCurrency(totalActual)}
        </div>
        <p className="text-slate-500 text-xs">{percentUsed.toFixed(1)}% of budget used</p>
      </div>
      ...
      <div className="bg-truehour-card border border-truehour-border rounded-lg p-6 hover:border-truehour-orange transition-colors">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className={`${remaining < 0 ? "bg-red-500/10" : "bg-orange-500/10"} p-2 rounded-lg`}>
            <svg
              className={`w-5 h-5 ${remaining < 0 ? "text-red-400" : "text-truehour-orange"}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <p className="text-slate-500 text-xs">{remaining < 0 ? "Over budget" : "Available to spend"}</p>
      </div>
    </div>
  );
}
