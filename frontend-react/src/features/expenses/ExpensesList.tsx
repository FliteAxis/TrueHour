...
export function ExpensesList({ expenses, budgetCards, onEditExpense }: ExpensesListProps) {
  ...
  return (
    <>
      ...
      {linkingExpenseId !== null && <div className="fixed inset-0 z-40" onClick={closeLinkDropdown} />}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col gap-1">
                          {(() => {
                            ...
                            return (
                              <span
                                className={`inline-flex items-center text-xs px-2.5 py-1 rounded ${colors.bg} ${colors.text} border ${colors.border} w-fit`}
                              >
                                {expense.category}
                              </span>
                            );
                          })()}
                          {expense.subcategory && <span className="text-xs text-slate-400">{expense.subcategory}</span>}
                        </div>
                      </td>
