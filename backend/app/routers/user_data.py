...
async def save_user_data(
    data: Optional[SaveDataRequest] = None,
        ...
        async with postgres_db.acquire() as conn:
            ...
            if data and data.budget_state:
                ...
                settings_row = await conn.fetchrow(
                    "SELECT id FROM user_settings ORDER BY id LIMIT 1"
                )
                ...
                if settings_row:
                    ...
                    await conn.execute("""
                        ...
                        WHERE id = $2
                    """, json.dumps(data.budget_state), settings_row['id'])
                else:
                    # Insert new row
                    await conn.execute("""
        ...
        async with postgres_db.acquire() as conn:
            ...
            if settings_row:
                ...
                if settings_row.get('budget_state'):
                    budget_state = json.loads(settings_row['budget_state']) if isinstance(settings_row['budget_state'], str) else settings_row['budget_state']
            ...
            if session_id:
                session_row = await conn.fetchrow(
                    "SELECT last_saved_at FROM user_sessions WHERE session_id = $1",
                    session_id
                )
                if session_row:
